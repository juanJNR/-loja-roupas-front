    ...
    async function cadastraCliente(data: Inputs) {
        ...
        try {
            ...
            if (response.status == 201) {
                ...
                // Exibe mensagem de erro específica se houver
                if (erro.erro) {
                    if (typeof erro.erro === 'string') {
                        toast.error(erro.erro)
                    } else if (erro.erro.issues) {
                        // Erros de validação do Zod
                        const mensagens = erro.erro.issues.map((issue: any) => issue.message).join('; ')
            ...
            <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
                <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        <h1 className="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-gray-400">
                            Cadastro de Cliente
                        </h1>
                          ...
                          onSubmit={handleSubmit(cadastraCliente)}>
                            ...
                            <div>
                                ...
                                <input type="password" id="password" placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required 
                                      {...register("senha")} />
                                <p className="mt-1 text-xs text-gray-500">Mínimo 8 caracteres, com maiúscula, minúscula, número e símbolo. Ex: Senha@123</p>
                            </div>
                            <div>
